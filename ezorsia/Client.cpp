...
bool Client::UpdateResolution() {
	...
	patches.WriteInt(dwCursorVectorVPos + 2, (unsigned int)floor(-m_nGameHeight / 2));//push -300				!!moves all interactable UI elements!!
	patches.WriteInt(dwCursorVectorHPos + 2, (unsigned int)floor(-m_nGameWidth / 2));	//push -400 ; CInputSystem::SetCursorVectorPos				!!moves all interactable UI elements!!
	patches.WriteInt(dwUpdateMouseLimitVPos + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(dwUpdateMouseLimitHPos + 1, m_nGameWidth);	//mov ecx,800 ; CInputSystem::UpdateMouse
	patches.WriteInt(dwCursorPosLimitVPos + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(dwCursorPosLimitHPos + 1, m_nGameWidth);	//mov eax,800 ; CInputSystem::SetCursorPos
	patches.WriteInt(dwViewPortHeight + 3, m_nGameHeight);//lea eax,[esi+eax-600]
	patches.WriteInt(dwViewPortWidth + 3, m_nGameWidth);	//lea eax,[ecx+eax-800]

	patches.WriteInt(dwToolTipLimitVPos + 1, m_nGameHeight - 1); //mov eax,599 ; CUIToolTip::MakeLayer
	patches.WriteInt(dwToolTipLimitHPos + 1, m_nGameWidth - 1); //mov eax,799 ; CUIToolTip::MakeLayer

	patches.WriteInt(dwTempStatToolTipDraw + 3, -m_nGameWidth + 6); //lea eax,[eax+ecx-797] ; CTemporaryStatView::ShowToolTip
	patches.WriteInt(dwTempStatToolTipFind + 3, -m_nGameWidth + 6); //lea eax,[eax+ecx-797] ; CTemporaryStatView::FindIcon
	patches.WriteInt(dwTempStatIconVPos + 2, (m_nGameHeight / 2) - 23);	//sub ebx,277 ; Skill icon buff y-pos
	patches.WriteInt(dwTempStatIconHpos + 3, (m_nGameWidth / 2) - 3);	//lea eax,[eax+esi+397] ; Skill icon buff x-pos
	patches.WriteInt(dwTempStatCoolTimeVPos + 2, (m_nGameHeight / 2) - 23);	//sub ebx,277 ; Skill icon cooltime y-pos
	patches.WriteInt(dwTempStatCoolTimeHPos + 3, (m_nGameWidth / 2) - 3);	//lea eax,[eax+esi+397] ; Skill icon cooltime x-pos

	patches.WriteInt(dwQuickSlotInitVPos + 1, m_nGameHeight + 1);//add eax,533
	...
	patches.WriteInt(dwAvatarMegaWidth + 1, m_nGameWidth); //push 800 ; CAvatarMegaphone ; CreateWnd

	patches.WriteInt(0x0043717B + 1, m_nGameHeight);//mov edi,600
	patches.WriteInt(0x00437181 + 1, m_nGameWidth);	//mov esi,800 ; CreateWnd
	patches.WriteInt(0x0053808B + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x00538091 + 1, m_nGameWidth);	//push 800 ; RelMove?
	patches.WriteInt(0x004CC160 + 3, m_nGameWidth);	//mov [ebp-16],800 ; CreateWnd
	patches.WriteInt(0x004CC2C5 + 2, m_nGameHeight);//cmp ecx,600
	patches.WriteInt(0x004CC2B0 + 1, m_nGameWidth);	//mov eax,800 ; CreateWnd
	patches.WriteInt(0x004D59B2 + 2, m_nGameHeight);//cmp ecx, height (preserve opcode and ModRM)
	patches.WriteInt(0x004D599D + 1, m_nGameWidth);	//mov eax,800 ; CreateWnd
	patches.WriteInt(0x0085F36C + 2, m_nGameWidth);	//cmp edx,800
	patches.WriteInt(0x0085F374 + 1, m_nGameWidth - 80);	//mov ecx,720 ; CreateDlg
	patches.WriteInt(0x008EBC58 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x008EBC3C + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x009966B5 + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x009966CA + 2, m_nGameWidth);	//cmp edi,800
	patches.WriteInt(0x009966D2 + 1, m_nGameWidth - 100);	//mov edx,700 ; CreateDlg
	patches.WriteInt(0x009A3E7F + 1, m_nGameHeight);//mov edx,600
	patches.WriteInt(0x009A3E72 + 1, m_nGameWidth);	//mov edx,800 ; CreateDlg
	//patches.WriteInt(0x0045B898 + 1, m_nGameHeight - 25);	//push 575
	patches.WriteInt(0x0045B898 + 1, m_nGameWidth - 225);	//push 575 ##ED  //smega x axis fade
	//patches.WriteInt(0x0045B97E + 1, m_nGameWidth);	//push 800 ; RelMove? ##REDUN
	//patches.WriteInt(0x004D9BD1 + 1, m_nGameWidth);	//push 800	; StringPool#1443 (BtMouseCilck)		//click ???related?? for tabs and numbers in cash shop
	//patches.WriteInt(0x004D9C37 + 1, m_nGameWidth);	//push 800	; StringPool#1443 (BtMouseCilck)		//click ???related?? for tabs and numbers in cash shop
	//patches.WriteInt(0x004D9C84 + 1, m_nGameWidth);	//push 800 ; StringPool#1443 (BtMouseCilck)		//click ???related?? for tabs and numbers in cash shop
	patches.WriteInt(0x005386F0 + 1, m_nGameHeight);//push 600
	...
	patches.WriteInt(0x008AA266 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x008AA26B + 1, m_nGameWidth);	//push 800 ; CreateWnd
	int winWidth = m_bEnableScaling ? m_nWindowWidth : m_nGameWidth;
	int winHeight = m_bEnableScaling ? m_nWindowHeight : m_nGameHeight;

	patches.WriteInt(0x009F6E99 + 1, winHeight);//push 600
	patches.WriteInt(0x009F6EA0 + 1, winWidth);	//push 800 ; StringPool#1162 (MapleStoryClass)

	patches.WriteInt(0x007CF48F + 1, m_nGameHeight);//mov eax,600 ;
	patches.WriteInt(0x007CF49D + 1, m_nGameWidth);	//mov eax,800 ; IWzVector2D::RelMove
	patches.WriteInt(0x008A12F4 + 1, m_nGameHeight);//mov eax,600 ;
	patches.WriteInt(0x008A1302 + 1, m_nGameWidth);	//mov eax,800 ; IWzVector2D::RelMove
	patches.WriteInt(0x007F257E + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x007F258F + 1, m_nGameWidth);	//push 800 ; CWnd::CreateWnd
	patches.WriteInt(0x0046B85C + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x0046B86A + 1, m_nGameWidth);	//mov eax,800 ; IWzVector2D::RelMove
	patches.WriteInt(0x009994D8 + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x009994ED + 2, m_nGameWidth);	//cmp edi,800
	patches.WriteInt(0x009994F5 + 1, m_nGameWidth - 100);	//mov edx,700 ; CreateDlg

	patches.WriteInt(0x0062FC4A + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0062FC4F + 1, m_nGameWidth);	//push 800 ; IWzGr2DLayer::Getcanvas
	patches.WriteInt(0x0062FE63 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0062FE68 + 1, m_nGameWidth);	//push 800 ; IWzGr2DLayer::Getcanvas
	patches.WriteInt(0x0062F9C6 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0062F9CB + 1, m_nGameWidth);	//push 800; (UI/Logo/Wizet)
	patches.WriteInt(0x0062F104 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x0062F109 + 1, m_nGameWidth);	//mov eax,800 ; Rectangle
	patches.WriteInt(0x006406D5 + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x006406C3 + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x0064050A + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x006404F8 + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x00640618 + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x00640690 + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x0064064B + 1, m_nGameHeight);//mov ecx,600
	patches.WriteInt(0x00640606 + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x0064067E + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x00640639 + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x0064043E + 1, (unsigned int)floor(m_nGameWidth / 2));	//mov edi,400
	patches.WriteInt(0x00640443 + 1, (unsigned int)floor(m_nGameHeight / 2));	//mov esi,300
	patches.WriteInt(0x00640626 + 1, (unsigned int)floor(m_nGameWidth / 2));	//add eax,400 ; bunch of modulus stuff

	patches.WriteInt(0x00641038 + 2, m_nGameHeight);//??possibly related to player display
	patches.WriteInt(0x0064103F + 2, m_nGameWidth);//??possibly related to player display
	patches.WriteInt(0x00641048 + 1, (unsigned int)floor(-m_nGameHeight / 2));	//mov esi,-300
	patches.WriteInt(0x00641050 + 1, (unsigned int)floor(-m_nGameWidth / 2));		//mov esi,-400 ;
	patches.WriteInt(0x00641A19 + 3, m_nGameHeight);//mov [ebp+28],600
	patches.WriteInt(0x00641A12 + 3, m_nGameWidth);	//mov [ebp+32],800 ; idk
	patches.WriteInt(0x00641B38 + 3, m_nGameHeight);//mov [ebp-32],600
	patches.WriteInt(0x00641B2E + 3, m_nGameWidth);	//mov [ebp-36],800 ; CAnimationDisplayer::SetCenterOrigin

	patches.WriteInt(0x006CD842 + 1, (unsigned int)floor(m_nGameWidth / 2));	//push 400 ; RelMove?

	patches.WriteInt(0x0059A0A2 + 6, (unsigned int)floor(m_nGameHeight / 2));	//mov [ebx+2364],300
	patches.WriteInt(0x0059A09C + 2, (unsigned int)floor(m_nGameWidth / 2));	//mov [esi],400	; CInputSystem::LoadCursorState
	...
	patches.WriteInt(0x0055BB35 + 1, (unsigned int)floor(-m_nGameWidth / 2));	//push -400 ; RelMove?

	patches.WriteInt(0x005A8B46 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005A8B56 + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005A9B42 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005A9B52 + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005AADAA + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005AADBA + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005ABC65 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005ABC75 + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005ACB29 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005ACB39 + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005C187E + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005C188E + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005C2D62 + 1, m_nGameHeight);//mov eax,600
	patches.WriteInt(0x005C2D72 + 1, m_nGameWidth);	//mov eax,800 ; RelMove?
	patches.WriteInt(0x005E3FA0 + 1, m_nGameHeight);//push 600

	patches.WriteInt(0x005F64DE + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;
	patches.WriteInt(0x005F6627 + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;
	if (bigLoginFrame) patches.WriteInt(0x005F464D + 1, m_nGameWidth - 165);//??related to login game frame

	patches.WriteInt(0x0060411C + 1, m_nGameHeight);//push 600
	//patches.WriteInt(0x00604126 + 1, floor(-m_nGameWidth / 2));	//push -300 //moves characters side to side on char select //unnecessary atm
	patches.WriteInt(0x0060F79B + 1, (m_nGameHeight / 2) - 201);//??possibly related to login utildlg
	patches.WriteInt(0x0060F7A4 + 1, (m_nGameHeight / 2) - 181);//??possibly related to login utildlg
	patches.WriteInt(0x0060F7AC + 1, (m_nGameWidth / 2) - 201);//??possibly related to login utildlg
	patches.WriteInt(0x0060F7B5 + 1, (m_nGameWidth / 2) - 181);//??possibly related to login utildlg
	patches.WriteInt(0x00613093 + 1, (m_nGameHeight / 2) - 200);//??likely related to login pop-up msg
	patches.WriteInt(0x00613099 + 1, (m_nGameWidth / 2) - 45);//??likely related to login pop-up msg
	patches.WriteInt(0x0061DAFF + 1, (m_nGameHeight / 2) - 150);//??likely related to login pop-up msg
	patches.WriteInt(0x0061DB08 + 1, (m_nGameHeight / 2) - 130);//??likely related to login pop-up msg
	patches.WriteInt(0x0061DB10 + 1, (m_nGameWidth / 2) - 201);//??likely related to login pop-up msg
	patches.WriteInt(0x0061DB19 + 1, (m_nGameWidth / 2) - 181);//??likely related to login pop-up msg

	patches.WriteInt(0x004372B1 + 1, (unsigned int)floor(-m_nGameHeight / 2));//push -300
	patches.WriteInt(0x004372B6 + 1, (unsigned int)floor(-m_nGameWidth / 2));	//push -400 ; RelMove?
	patches.WriteInt(0x006CE3AB + 1, m_nGameWidth);	//push 800
	patches.WriteInt(0x007E1CB7 + 1, m_nGameWidth);	//push 800
	patches.WriteInt(0x008D82F5 + 1, m_nGameHeight - 22);	//push 578
	patches.WriteInt(0x008D82FA + 1, m_nGameWidth);	//push 800 ; CreateWnd?
	patches.WriteInt(0x00935870 + 1, (unsigned int)floor(m_nGameHeight / 2));	//push 300
	patches.WriteInt(0x0093586B + 1, m_nGameWidth);	// push 800 ; RelMove? (Skills)
	patches.WriteInt(0x009DFD5C + 1, m_nGameWidth);	//mov ecx,800
	patches.WriteInt(0x009DFED2 + 1, m_nGameHeight);//mov ecx,600	; IWzVector2D::RelMove
	patches.WriteInt(0x009F6ADD + 1, (unsigned int)floor(m_nGameHeight / 2)); //push 300 ; MapleStoryClass
	patches.WriteInt(0x006D50D8 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0074BAA9 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0074B951 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0074B4A2 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x0074B3B7 + 1, m_nGameHeight);//push 600
	patches.WriteInt(0x006421B3 + 1, m_nGameHeight);//push 600 ; CSoundMan::PlayBGM

	patches.WriteInt(0x0059EB49 + 1, m_nGameHeight);//push 600 ; CSoundMan::PlayBGM
	patches.WriteInt(0x008D247B + 1, m_nGameHeight - 33);	//push 567 ; IWzVector2D::RelMove
	patches.WriteInt(0x008DEB93 + 1, m_nGameHeight - 20);	//push 580
	patches.WriteInt(0x008DEE2F + 1, m_nGameHeight - 20);	//push 580
	patches.WriteInt(0x008D2765 + 1, m_nGameHeight - 19);	//push 581
	patches.WriteInt(0x008DA11C + 1, m_nGameHeight - 19);	//push 581 临时经验条
	patches.WriteInt(0x008D29B4 + 1, m_nGameHeight - 19);	//push 581
	patches.WriteInt(0x008D8BFE + 1, m_nGameHeight - 19);	//push 581
	...
	patches.WriteInt(0x004EDB78 + 1, m_nGameHeight);//mov ecx,600 ; CreateWnd
	patches.WriteInt(0x004EDAD8 + 1, m_nGameWidth);	//mov ecx,800
	...
	patches.WriteInt(0x009E2E85 + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300		overall screen visible UI scaling
	patches.WriteInt(0x009E2E8B + 1, (unsigned int)floor(-m_nGameWidth / 2));		//push -400 ;	overall screen visible UI scaling

	patches.WriteInt(0x0093519A + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;
	patches.WriteInt(0x00954433 + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;
	patches.WriteInt(0x00981555 + 1, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;
	patches.WriteInt(0x00981F7A + 2, (unsigned int)floor(-m_nGameHeight / 2));	//push -300 ;

	patches.WriteInt(0x0066BACE + 2, (unsigned int)floor(-m_nGameWidth / 2));		//and ecx,-400
	patches.WriteInt(0x009B76BD + 3, (unsigned int)floor(-m_nGameHeight / 2));	//push -300
	patches.WriteInt(0x009B76CB + 3, (unsigned int)floor(m_nGameHeight / 2));		//push 300

	patches.WriteInt(0x009F7078 + 1, m_nGameHeight);//??related to application dimensions	//(ragezone release merge)//thanks mr mr of ragezone for these addresses
	patches.WriteInt(0x009F707D + 1, m_nGameWidth);//??related to application dimensions

	patches.WriteInt(0x0058C8A6 + 1, m_nGameWidth);//??

	patches.WriteInt(0x004EDABF + 1, m_nGameHeight);//??

	patches.WriteInt(0x00991854 + 1, m_nGameHeight);//??unknown cwnd function
	patches.WriteInt(0x0099185F + 1, (m_nGameWidth / 2) - 134);//??unknown cwnd function
	patches.WriteInt(0x00991867 + 1, (m_nGameWidth / 2) - 133);//??unknown cwnd function
	patches.WriteInt(0x00992BA7 + 1, (unsigned int)floor(m_nGameWidth / 2));//??unknown cwnd function, possibly related to cutildlg
	patches.WriteInt(0x00992BAC + 1, (unsigned int)floor(m_nGameHeight / 2));//??unknown cwnd function, possibly related to cutildlg

	patches.WriteInt(0x007E1E07 + 2, m_nGameWidth);//??related to displaying server message at top of screen
	patches.WriteInt(0x007E19CA + 2, m_nGameWidth);//??related to displaying server message at top of screen

	patches.WriteInt(0x005362B2 + 1, (m_nGameWidth / 2) - 129);//??related to boss bar
	patches.WriteInt(0x005364AA + 2, (m_nGameWidth / 2) - 128);//??related to boss bar

	patches.WriteInt(0x00592A08 + 1, (m_nGameWidth / 2) - 125);//??likely related to mouse pos

	patches.WriteInt(0x00621226 + 1, (m_nGameWidth / 2) - 216);//??possibly related to logo
	patches.WriteByte(0x0062121E + 1, 0x01);//??possibly related to logo

	patches.WriteInt(0x008C069F + 1, (m_nGameHeight / 2) - 14);//??related to status bar
	patches.WriteInt(0x008C06A4 + 1, (m_nGameWidth / 2) - 158);//???related to status bar

	patches.WriteInt(0x00A24D0B + 1, (m_nGameWidth / 2) - 129);//??

	patches.WriteInt(0x00BE273C, 128);//??
	//patches.WriteByte(0x008D1790 + 2, 0x01); //related to quickslots area presence		 originally 1U but changed because unsigned int crashes it after char select
	patches.WriteByte(0x0089B636 + 2, 0x01); //related to exp gain/item pick up msg, seems to affect msg height ! originally 1U but changed because unsigned int crashes it after char select
	patches.WriteByte(0x00592A06 + 1, 0x01);//???likely related to mouse pos

	patches.WriteInt(0x00744EB4 + 1, m_nGameWidth);//??related to in-game taking screenshot functionality
	patches.WriteInt(0x00744EB9 + 1, m_nGameHeight);//??related to in-game taking screenshot functionality
	patches.WriteInt(0x00744E2A + 1, 3 * m_nGameWidth * m_nGameHeight);//??related to in-game taking screenshot functionality
	patches.WriteInt(0x00744E43 + 1, m_nGameWidth * m_nGameHeight);//??related to in-game taking screenshot functionality
	patches.WriteInt(0x00744DA6 + 1, 4 * m_nGameWidth * m_nGameHeight);//??related to in-game taking screenshot functionality

	patches.WriteInt(0x00897BB4 + 1, (m_nGameWidth / 2) - 143);//??related to exp gain/item pick up msg

	if (WindowedMode) {
		unsigned char forced_window[] = { 0xb8, 0x00, 0x00, 0x00, 0x00 }; //force window mode	//thanks stelmo for showing me how to do this
		patches.WriteByteArray(0x009F7A9B, forced_window, sizeof(forced_window));//force window mode
	}
	if (RemoveLogos) {
		patches.FillBytes(0x0062EE54, 0x90, 21);	//no Logo @launch //Thanks Denki!!
	}

	int msgAmntOffset, msgAmnt; msgAmnt = MsgAmount; msgAmntOffset = msgAmnt * 14;

	patches.WriteInt(0x0089B639 + 1, m_nGameHeight - 6 - msgAmntOffset - 80);//inventory/exp gain y axis //####hd100 //90
	patches.WriteInt(0x0089B6F7 + 1, m_nGameWidth - 405);//inventory/exp gain x axis //310 //####hd415 //405

	patches.WriteInt(0x0089AF33 + 1, 400);//length of pick up and exp gain message canvas //found with help from Davi
	patches.WriteInt(0x0089B2C6 + 1, 400);//address to move the message in the canvas adjusted above to the center of the new canvas  //thanks chris

	patches.WriteInt(0x0089AEE2 + 3, msgAmnt);//moregainmsgs part 1
	MoreGainMsgsOffset = msgAmnt;	//param for ccmoregainmssgs
	patches.CodeCave(ccMoreGainMsgs, dwMoreGainMsgs, MoreGainMsgsNOPs); //moregainmsgs part 2
	MoreGainMsgsFadeOffset = 15000;	//param for ccmoregainmssgsFade
	patches.CodeCave(ccMoreGainMsgsFade, dwMoreGainMsgsFade, MoreGainMsgsFadeNOPs); //moregainmsgsFade
	MoreGainMsgsFade1Offset = 255 * 4 / 3;	//param for ccmoregainmssgsFade
	patches.CodeCave(ccMoreGainMsgsFade1, dwMoreGainMsgsFade1, MoreGainMsgsFade1NOPs); //moregainmsgsFade1

	patches.WriteInt(0x0045B337 + 1, m_nGameWidth);//related to smega display  //likely screen area where pop up starts for smega
	patches.WriteInt(0x0045B417 + 1, m_nGameWidth - 225);//smega with avatar x axis for duration on screen

	patches.WriteInt(0x007C2531 + 1, m_nGameHeight - 80);//??

	//patches.WriteInt(0x0089B796 + 2, m_nGameHeight - 18);//???related to exp gain/item pick up msg
	//patches.WriteInt(0x0089BA03 + 1, m_nGameHeight - 96); //??related to exp gain/item pick up msg
	//patches.WriteInt(0x008D3F73 + 1, m_nGameHeight - 93);//bottom frame, white area
	//patches.WriteInt(0x008D3FE5 + 1, m_nGameHeight - 93);//bottom frame, grey area
	//patches.WriteInt(0x008D8353 + 1, m_nGameHeight - 46); //bottom frame, character level
	//patches.WriteInt(0x008D83D1 + 1, m_nGameHeight - 55); //role
	//patches.WriteInt(0x008D8470 + 1, m_nGameHeight - 40); //name of character

	//patches.WriteInt(0x008DE850 + 1, 580);//quickslotcheckX//interactivity of bottom buttoms
	//patches.WriteInt(0x008DE896 + 1, 647);//quickslotcheckX//interactivity of bottom buttoms
	//patches.WriteInt(0x008DE82B + 1, 507);///quickslotcheckY //interactivity of bottom buttoms

	//patches.WriteInt(0x008DA11C + 1, m_nGameHeight - 19);//??likely various status bar UI components
	//patches.WriteInt(0x008DA3D4 + 1, m_nGameHeight - 56); //exphpmp % labels
	//patches.WriteInt(0x008DA463 + 1, m_nGameHeight - 51); //stat bar gradient or bracket
	//patches.WriteInt(0x008DA4F2 + 1, m_nGameHeight - 51);//stat bar gradient or bracket
	//patches.WriteInt(0x008DA61B + 1, m_nGameHeight - 56);//??likely various status bar UI components

	//patches.WriteInt(0x008DA90F + 1, m_nGameHeight - 51);//brackets for stat numbers
	//patches.WriteInt(0x008DA9C6 + 1, m_nGameHeight - 51);
	//patches.WriteInt(0x008DAC3F + 1, m_nGameHeight - 51);
	//patches.WriteInt(0x008DACF1 + 1, m_nGameHeight - 51);
	//patches.WriteInt(0x008DAF64 + 1, m_nGameHeight - 51);

	//patches.WriteInt(0x008DFA6F + 1, m_nGameHeight - 81);//chat box selection, dragging box size, minus plus sign, typing interac
	//patches.WriteInt(0x008DFB01 + 1, m_nGameHeight - 81);
	//patches.WriteInt(0x008DFBA5 + 1, m_nGameHeight - 80);
	//patches.WriteInt(0x008DFC10 + 1, m_nGameHeight - 85);

	//patches.WriteInt(0x008D4AFB + 1, m_nGameHeight - 91); //is for the little grab/resize bar on it (I think)??
	//patches.WriteInt(0x008D4C1F + 1, m_nGameHeight - 90);//??likely various status bar UI components
	//patches.WriteInt(0x008D4CDD + 1, m_nGameHeight - 20);//??likely various status bar UI components
	//patches.WriteInt(0x008D4BBC + 6, m_nGameHeight - 114);//??likely various status bar UI components
	//patches.WriteInt(0x008D4C47 + 1, m_nGameHeight - 87);//minimized chat box frame
	//patches.WriteInt(0x008D628B + 1, m_nGameHeight - 91); //is for the background for the text area.??
	//patches.WriteInt(0x008D6300 + 1, m_nGameHeight - 90); //is for the scroll bar on the chat text area.??
	//patches.WriteInt(0x008D4B6D + 1, m_nGameHeight - 90);//scroll bar of chat
	//patches.WriteInt(0x008D276A + 1, m_nGameHeight - 19);//??likely various status bar UI components

	//patches.WriteInt(0x008D7778 + 3, m_nGameHeight - 42);//???likely various status bar UI components
	//patches.WriteInt(0x008D7785 + 3, m_nGameHeight - 26);//??likely various status bar UI components
	//patches.WriteInt(0x008D783A + 3, m_nGameHeight - 41);//??likely various status bar UI components
	//patches.WriteInt(0x008D7847 + 3, m_nGameHeight - 26);//??likely various status bar UI components

	//patches.WriteInt(0x008D2FAE + 1, m_nGameHeight - 57); //bottom 4 large buttons
	//patches.WriteInt(0x008D3056 + 1, m_nGameHeight - 57);
	...
	patches.WriteInt(0x0052307E + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523359 + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523595 + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up //quest complete y axis
	patches.WriteInt(0x0052378B + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x0052397D + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523BB1 + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523DA5 + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523FA3 + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x005243DB + 1, m_nGameHeight - 92 - 10);// various requests like party, guild, friend, family, invites that pop up
	patches.WriteInt(0x00523154 + 1, m_nGameHeight - 102 - 10);//?? various requests like party, guild, friend, family, invites that pop up

	patches.WriteInt(0x0052418C + 1, m_nGameHeight - 102 - 10);//party quest available pop-up y axis		my first address find own my own

	patches.WriteInt(0x00523092 + 1, 423);// 交易申请气泡x坐标
	patches.WriteInt(0x0052336D + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up //Party Invite
	patches.WriteInt(0x00522E79 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up //friend request
	patches.WriteInt(0x00522C87 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up	// Guild Invite
	//patches.WriteInt(0x005235A9 + 1, m_nGameWidth - 942);//various requests like party, guild, friend, family, invites that pop up	// Quest Complete, currently unneeded as working without it
	patches.WriteInt(0x0052379F + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up//??
	patches.WriteInt(0x00523991 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up/??
	patches.WriteInt(0x00523BC5 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up/??
	patches.WriteInt(0x00523DC5 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up// ??
	patches.WriteInt(0x00523FB7 + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up// ??
	patches.WriteInt(0x005243EF + 1, m_nGameWidth - 942 + 26 + 37 + 62);//various requests like party, guild, friend, family, invites that pop up//??

	//patches.WriteInt(0x008D326E + 1, m_nGameHeight - 85); //smol buttoms right of chat box (all - 85 ones)
	//patches.WriteInt(0x008D32F5 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D337C + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D3403 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D348A + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D3586 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D3696 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D4058 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008DF903 + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008DFFCF + 1, m_nGameHeight - 85);
	//patches.WriteInt(0x008D40CE + 1, m_nGameHeight - 81);//smol buttoms right of chat box (all - 85 ones)

	//patches.CodeCave(PositionBossBarY2, 0x007E169B, 6);//boss bar, check for server msg, looking in wrong address...
	//if (serverMessageExists != 0) 
	//{
	//	patches.CodeCave(PositionBossBarY, dwBossBar, 7);//boss bar normal position without server msg
	//}
	//else {
	//	patches.CodeCave(PositionBossBarY1, dwBossBar, 7);//boss bar with server msg
	//}

	patches.WriteInt(0x00533B03 + 1, m_nGameWidth - 15);	//boss bar	extend to window
	patches.WriteInt(0x00534370 + 1, m_nGameWidth - 22);	//boss bar	extend to window
	...
	//patches.CodeCave(CashShopFix, dwCashFix, 6);//cash shop fix for frame area //moves frame to top left (not used rn)
	...
	patches.CodeCave(CashShopFix1, dwCashFix1, dwCashFix1NOPs);
	nHeightOfsetted2 = 104; nWidthOfsetted2 = 256; nTopOfsetted2 = 318 + myHeight; nLeftOfsetted2 = -1 + myWidth; //parameters for fix2
	patches.CodeCave(CashShopFix2, dwCashFix2, dwCashFix2NOPs);
	nHeightOfsetted3 = 163; nWidthOfsetted3 = 246; nTopOfsetted3 = 426 + myHeight; nLeftOfsetted3 = 0 + myWidth; //parameters for fix3
	patches.CodeCave(CashShopFix3, dwCashFix3, dwCashFix3NOPs);
	nHeightOfsetted4 = 78; nWidthOfsetted4 = 508; nTopOfsetted4 = 17 + myHeight; nLeftOfsetted4 = 272 + myWidth; //parameters for fix4
	patches.CodeCave(CashShopFix4, dwCashFix4, dwCashFix4NOPs);
	nHeightOfsetted5 = 430; nWidthOfsetted5 = 412; nTopOfsetted5 = 95 + myHeight; nLeftOfsetted5 = 275 + myWidth; //parameters for fix5
	patches.CodeCave(CashShopFix5, dwCashFix5, dwCashFix5NOPs);	//main part of shop, item listings	//thanks angel for stuff that helped
	nHeightOfsetted6 = 358; nWidthOfsetted6 = 90; nTopOfsetted6 = 157 + myHeight; //parameters for fix6
	patches.CodeCave(CashShopFix6, dwCashFix6, dwCashFix6NOPs);//code cave 6 //best sellers
	patches.WriteInt(0x004694BA + 1, myWidth + 690);//nleft, actual drawn part	//best sellers
	nHeightOfsetted7 = 56; nWidthOfsetted7 = 545; nTopOfsetted7 = 530 + myHeight; nLeftOfsetted7 = 254 + myWidth; //parameters for fix7
	patches.CodeCave(CashShopFix7, dwCashFix7, dwCashFix7NOPs);
	nHeightOfsetted8 = 22; nWidthOfsetted8 = 89; nTopOfsetted8 = 97 + myHeight; nLeftOfsetted8 = 690 + myWidth; //parameters for fix8
	patches.CodeCave(CashShopFix8, dwCashFix8, dwCashFix8NOPs);
	patches.CodeCave(CashShopFixOnOff, dwCashFixOnOff, dwCashFixOnOffNOPs);	//fix for preview On/Off button not being accurate on entering cash shop //thanks windyboy
	...
	//patches.CodeCave(loginFrameFix, dwloginFrameFix, loginFrameFixNOPs); //failed login frame fix =(

	if (!bigLoginFrame) {
		nTopOfsettedVerFix = 10 + myHeight; nLeftOfsettedVerFix = 645 + myWidth; //parameters for fix version number
		patches.CodeCave(VersionNumberFix, dwVersionNumberFix, dwVersionNumberFixNOPs);	//game version number fix //use this if you use no frame or default client frame
	}
	...
	if (!bigLoginFrame) {
		...
		patches.CodeCave(ccLoginBackCanvasFix, dwLoginBackCanvasFix, LoginBackCanvasFixNOPs);	//world select buttons fix		//thank you teto for pointing out my error in finding the constructor

		//yOffsetOfLoginBackBtnFix = 300 + myHeight; xOffsetOfLoginBackBtnFix = 0 + myWidth;	//para for back button
		//patches.CodeCave(ccLoginBackBtnFix, dwLoginBackBtnFix, LoginBackBtnFixNOPs); //back button on world select //unnecessary as buttons move with canvas
		...
		patches.CodeCave(ccLoginViewRecFix, dwLoginViewRecFix, LoginViewRecFixNOPs);	//world ViewRec fix

		a1x = 0 + myWidth; a2x = -149 + myWidth; a2y = 0 + myHeight; a3 = 25; a1y = -250; //a4 = 0;	//LoginDescriptor params
		patches.WriteInt(0x0060D849 + 1, 300 + a1y); //speed 1	//temporary fix by increasing the speed of display until i get good enough at procedural programming
		...
		patches.CodeCave(ccLoginDescriptorFix, dwLoginDescriptorFix, LoginDescriptorFixNOPs);	//world LoginDescriptor fix
	}
	...
	patches.CodeCave(ccMuruengraidPlayer, dwMuruengraidPlayer, MuruengraidPlayerNOPs);	//muruengraid scaling
	yOffsetOfMuruengraidClock = 26 + dojangYoffset; xOffsetOfMuruengraidClock = 400 + myWidth; //params
	patches.CodeCave(ccMuruengraidClock, dwMuruengraidClock, MuruengraidClockNOPs);	//muruengraid scaling
	yOffsetOfMuruengraidMonster = 50 + dojangYoffset; xOffsetOfMuruengraidMonster = 631 + myWidth; //params
	patches.CodeCave(ccMuruengraidMonster, dwMuruengraidMonster, MuruengraidMonsterNOPs);	//muruengraid scaling
	yOffsetOfMuruengraidMonster1 = 32 + dojangYoffset; xOffsetOfMuruengraidMonster1 = 317 + myWidth; //params
	patches.CodeCave(ccMuruengraidMonster1, dwMuruengraidMonster1, MuruengraidMonster1NOPs);	//muruengraid scaling
	yOffsetOfMuruengraidMonster2 = 32 + dojangYoffset; xOffsetOfMuruengraidMonster2 = 482 + myWidth; //params
	patches.CodeCave(ccMuruengraidMonster2, dwMuruengraidMonster2, MuruengraidMonster2NOPs);	//muruengraid scaling
	yOffsetOfMuruengraidEngBar = 86 + dojangYoffset + customEngY; xOffsetOfMuruengraidEngBar = 17 + myWidth + customEngX; //params
	patches.CodeCave(ccMuruengraidEngBar, dwMuruengraidEngBar, MuruengraidEngBarNOPs);	//muruengraid scaling
	yOffsetOfMuruengraidEngBar1 = 130 + dojangYoffset + customEngY; xOffsetOfMuruengraidEngBar1 = 20 + myWidth + customEngX; //params
	patches.CodeCave(ccMuruengraidEngBar1, dwMuruengraidEngBar1, MuruengraidEngBar1NOPs);	//muruengraid scaling
	yOffsetOfMuruengraidEngBar2 = 80 + dojangYoffset + customEngY; xOffsetOfMuruengraidEngBar2 = 9 + myWidth + customEngX; //params
	patches.CodeCave(ccMuruengraidEngBar2, dwMuruengraidEngBar2, MuruengraidEngBar2NOPs);	//muruengraid scaling
	yOffsetOfMuruengraidClearRoundUI = 260 + myHeight; xOffsetOfMuruengraidClearRoundUI = 400 + myWidth; //params
	patches.CodeCave(ccMuruengraidClearRoundUI, dwMuruengraidClearRoundUI, MuruengraidClearRoundUINOPs);	//muruengraid scaling
	//yOffsetOfMuruengraidTimerCanvas = 28 + dojangYoffset; xOffsetOfMuruengraidTimerCanvas = 112 + myWidth; //params
	//patches.CodeCave(ccMuruengraidTimerCanvas, dwMuruengraidTimerCanvas, MuruengraidTimerCanvasNOPs);	//muruengraid scaling
	...
	//patches.CodeCave(ccMuruengraidTimerMinutes, dwMuruengraidTimerMinutes, MuruengraidTimerMinutesNOPs);	//muruengraid scaling
	//yOffsetOfMuruengraidTimerSeconds = 0 + dojangYoffset; xOffsetOfMuruengraidTimerSeconds = 68 + myWidth; //params
	//patches.CodeCave(ccMuruengraidTimerSeconds, dwMuruengraidTimerSeconds, MuruengraidTimerSecondsNOPs);	//muruengraid scaling
	yOffsetOfMuruengraidTimerBar = 16 + dojangYoffset; xOffsetOfMuruengraidTimerBar = 345 + myWidth; //params
	patches.CodeCave(ccMuruengraidTimerBar, dwMuruengraidTimerBar, MuruengraidTimerBarNOPs);	//muruengraid scaling
	xOffsetOfMuruengraidMonster1_2 = 318 + myWidth; //params	//finally fixed this bugger
	patches.CodeCave(ccMuruengraidMonster1_2, dwMuruengraidMonster1_2, MuruengraidMonster1_2NOPs);	//muruengraid scaling
	...
	//patches.WriteInt(0x008C4AB3 + 1, myStatsWindowOffset); //stat window ty resinate
	//patches.WriteInt(0x008C510A + 1, myStatsWindowOffset1); //stat window ty resinate
	
	//const char* testString = "RoSWzFile"; patches.WriteString(0x00B3F434, testString);//testing
	//patches.WriteInt(0x009F74EA + 3, 16); //testing
	//patches.WriteInt(0x008C4286 + 1, 400); //testing
	//patches.WriteInt(0x00780743 + 3, 400); //testing
	//patches.WriteByte(0x004289C0 + 1, 99); //testing
	//patches.FillBytes(0x00485C01, 0x90, 2);
	//patches.FillBytes(0x00485C21, 0x90, 2);

	//patches.CodeCave(testingCodeCave, dwTesting, TestingNOPs); //testing
	//patches.CodeCave(testingCodeCave2, dwTesting2, Testing2NOPs); //testing
	//patches.CodeCave(testingCodeCave3, dwTesting3, Testing3NOPs); //testing
	//patches.CodeCave(testingCodeCave4, dwTesting4, Testing4NOPs); //testing

	darkCircleX = m_nGameWidth / 2 - 163;
	darkCircleY = m_nGameHeight / 2 - 190;
	patches.CodeCave(darkMap1cc, 0x0055BEE6, 9);
	patches.CodeCave(darkMap2cc, 0x0055C07F, 12);
	patches.CodeCave(darkMap3cc, 0x0055C1C5, 13);
	std::string error;
	const bool applied = patches.Apply(error);
	ResolutionPatch::Report(applied, error);
	return applied;
	} catch (const std::exception& error) {
		ResolutionPatch::Report(false, error.what());
		return false;
	}
}
