...
try {
    ...
    & $compiler @common /DWORLD_VIEWPORT_TESTING (Join-Path $repoRoot 'tests\SceneryViewportTests.cpp') (Join-Path $repoRoot 'ezorsia\WorldViewport.cpp') ('/Fo' + $testDir + '\') /link /MACHINE:X86 $detours oleaut32.lib user32.lib ('/OUT:' + (Join-Path $testDir 'SceneryViewportTests.exe'))
    if ($LASTEXITCODE -ne 0) { throw 'Scenery viewport test build failed' }
    & (Join-Path $testDir 'SceneryViewportTests.exe') (Join-Path $repoRoot 'tests\fixtures\map-scenery.txt')
    if ($LASTEXITCODE -ne 0) { throw 'Scenery viewport tests failed' }
    & $compiler @common /DWORLD_VIEWPORT_TESTING (Join-Path $repoRoot 'tests\GroundCameraTests.cpp') (Join-Path $repoRoot 'ezorsia\WorldViewport.cpp') ('/Fo' + $testDir + '\') /link /MACHINE:X86 $detours oleaut32.lib user32.lib ('/OUT:' + (Join-Path $testDir 'GroundCameraTests.exe'))
    if ($LASTEXITCODE -ne 0) { throw 'Ground camera test build failed' }
    & (Join-Path $testDir 'GroundCameraTests.exe')
    if ($LASTEXITCODE -ne 0) { throw 'Ground camera tests failed' }
